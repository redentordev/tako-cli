...
func TestServerStatusOverUnixSocket(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		...
		if err == nil {
			...
			if len(status.Capabilities) != 7 || status.Capabilities[0] != CapabilityContainerArgvV1 || status.Capabilities[1] != CapabilityContainerRuntimeControlsV1 || status.Capabilities[2] != CapabilityImageBuildOptionsV1 || status.Capabilities[3] != CapabilityExecOneOffControlsV1 || status.Capabilities[4] != CapabilityServiceFilesV1 || status.Capabilities[5] != CapabilityProxyTrustedProxiesV1 || status.Capabilities[6] != CapabilityProxyCertsV1 {
