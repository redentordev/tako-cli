...
if command -v curl &> /dev/null; then
    ...
    if [ "$HTTP_CODE" = "200" ] || [ "$HTTP_CODE" = "301" ] || [ "$HTTP_CODE" = "302" ]; then
        ...
        echo "   ✗ 404 Not Found - Service might not be deployed or proxy not configured"
...
echo "   → Check firewall: sudo ufw allow 80/tcp && sudo ufw allow 443/tcp && sudo ufw allow 443/udp"
...
echo "   → Check access logs: tako access --tail 100"
echo ""
echo "3. 502/503 Bad Gateway:"
echo "   → Service not healthy: tako logs --service plausible"
echo "   → Check dependencies: tako logs --service postgres"
echo "   → Check migrations ran: tako logs --service plausible | grep migrate"
echo ""
echo "4. SSL Certificate issues:"
echo "   → Wait 1-2 minutes after first deployment"
echo "   → Check proxy logs for Let's Encrypt errors"
echo ""
echo "Need more help? Run:"
echo "  tako ps"
echo "  tako access --tail 100"
echo "  tako logs --service plausible --tail 100"
echo "=================================================="
