...
- `web-spread`: spreads three replicas across available nodes.
...
Global placement should create one `monitoring-agent` container per configured node. Spread placement should balance slots across reachable nodes. Pinned placement should keep the service on the named node.
