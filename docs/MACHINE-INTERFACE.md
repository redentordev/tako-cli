...
| Human-only by design | `init`, `platform init`, `config explain`, `monitor`, `env`, `secrets init\|set\|delete\|fetch\|import` (local mutations; `fetch`/`import` print redacted command-local JSON), `upgrade` (CLI self-update; `upgrade servers` keeps the full contract) |
| Infrastructure-only | `takod run`, hidden `platform worker run`, and hidden internal E2E helpers |
